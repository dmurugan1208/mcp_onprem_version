...
# ============================================================================
# APIKeyRepository — API key management (JSON-backed, thread-safe)
# ============================================================================

class APIKeyRepository:
    """Reads and caches API keys from sajhamcpserver/config/apikeys.json.

    Thread-safe with lock. Supports reload() to re-read from disk.
    Default path: ../sajhamcpserver/config/apikeys.json (relative to agent/)
    """
    ...
    def __init__(self, config_path: str = None):
        ...
        Args:
            config_path: Path to apikeys.json. If None, uses default relative path.
        """
        if config_path is None:
            agent_dir = os.path.dirname(os.path.abspath(__file__))
            config_path = os.path.join(agent_dir, '..', 'sajhamcpserver', 'config', 'apikeys.json')
    ...
    def reload(self) -> None:
        """Re-read apikeys.json from disk."""
        with self._lock:
            try:
                with open(self._config_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self._keys = data if isinstance(data, list) else data.get('apikeys', [])
            except FileNotFoundError:
                self._keys = []
            except Exception:
                pass

    def find(self, key_id: str) -> Optional[dict]:
        """Return API key dict or None if not found."""
        with self._lock:
            for k in self._keys:
    ...
    def find_by_hash(self, key_hash: str) -> Optional[dict]:
        """Return API key by hash (for auth validation)."""
        with self._lock:
            for k in self._keys:
    ...
    def list(self) -> list:
        """Return all API key dicts (redacted)."""
        with self._lock:
            return list(self._keys)

    def list_active(self) -> list:
        """Return all non-revoked, non-expired API keys."""
        import datetime
        with self._lock:
            now = datetime.datetime.utcnow()
            return [
                k for k in self._keys
                if not k.get('revoked', False) and (
                    k.get('expires_at') is None or
                    datetime.datetime.fromisoformat(k.get('expires_at', '')) > now
                )
            ]
